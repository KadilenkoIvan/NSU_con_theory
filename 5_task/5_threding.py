...
def fun_thread_safe_predict(frame_queue: queue.Queue, event_stop: threading.Event, output_queue: queue.Queue):
...
def main(input_video_path, num_of_threads, output_video_path):
    ...
    thread_read = threading.Thread(target=fun_thread_read, args=(input_video_path, frame_queue, event_stop, ))
    thread_read.start()
    start_t = time.monotonic()
    for _ in range(num_of_threads):
        threads.append(threading.Thread(target=fun_thread_safe_predict, args=(frame_queue, event_stop, output_queue, )))
...
if __name__ == "__main__":
    ...
    main(args.input_video_path, args.threads, args.output_video_path)